...
void ModerateComputer::rollDice(Player *p) {
    ...
    do{
        ...
        for (auto i : dice){
            ...
            if (i.second.getState() != "Heal" && i.second.getState() != "Star"){
...
void ModerateComputer::resolveDice(Player* p, vector<Player>* players, Map* m){
...
void ModerateComputer::move(Player* p, vector<Player>* players, Map* m){
    string new_region;
    Region *region = p->getRegion();
...
void ModerateComputer::buyCards(Player* p, Deck<Card>* deck){
